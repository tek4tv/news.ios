...
class PageAudioVC: ButtonBarPagerTabStripViewController {
    ...
    override func viewControllers(for pagerTabStripController: PagerTabStripViewController) -> [UIViewController] {
        ...
        if listPage.count != 0 {
            for i in listPage {
                let child_1 = UIStoryboard(name: "Main", bundle: nil).instantiateViewController(withIdentifier: "ContentPageVideo") as! ContentPageVideo
                child_1.name = i.name
                child_1.id = i.id
                child_1.bg = self.bg
